...
def seg2diffgrads(label,topt):
    # input: (n, y, x) for 2D data & (n, z, y, x) for 3D data
    # output: (n, 4, y, x) for 2D data & (n, 4, z, y, x) for 3D data
    # flows[i][0] is labels[i], flows[i][1] is Y flow, flows[i][2] is X flow, flows[i][3] is probability map
    
    device = torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')
    # label are fixed here to be unique, so they need to be passed back
    # make sure labels are unique!
    # TODO: deal with this
    label_temp = label
    label = fastremap.renumber(label, in_place=True)[0]
    print((label_temp == label).all() )
    ...
    if masks.ndim==3:
        ...
        #     mu[[0,1], :, :, x] += mu0
        mu = mu.transpose(1,0,2,3)
        label_ = np.expand_dims(label,axis=0)
        flows = np.concatenate((label_,mu,label_>0.5), axis=0).astype(np.float32)
    elif masks.ndim==2:
        # concatenate vector flows & cell probability binary maps
        mu, mu_c,centers = masks_to_flows(masks, device)
        flows = np.expand_dims(np.concatenate((label,mu,label>0.5), axis=0).astype(np.float32),axis=1)
    else:
        raise ValueError('expected only takes 2D or 3D arrays in seg2diffgrads')
